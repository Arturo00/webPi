...
namespace PisoEstudiantes.Controllers
{
    public class AccountController : Controller
    {
        ...
        public ActionResult Register(RegisterViewModel model, string returnUrl)
        {
            if (ModelState.IsValid)
            {
                //(string email, string name, string phone, string age, string leaseholder, string surname, string password, string gender)
                User u = new User(model.Name, model.Surname, model.Email, model.Leaseholder, model.Age, model.Gender, model.Phone, model.Password);
                var user = userModel.register(u);
            }
            else
            {
