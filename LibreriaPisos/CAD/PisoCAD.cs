...
namespace LibreriaPisos.CAD
{
    public static class PisoCAD
    {
        ...
        {
            int value = -1;
            string SQL = string.Format("INSERT INTO {0}( IDUSER, PAIS, CIUDAD, POBLACION, CALLE, CODPOS, PRECIO, OCUPANTES, ALQUILADO, DESCRIPCION, PUNTUACION, ELIMINADO) " +
            " VALUES( @IDUSER, @PAIS, @CIUDAD, @POBLACION, @CALLE, @CODPOS, @PRECIO, @OCUPANTES, @ALQUILADO, @DESCRIPCION, @PUNTUACION, @ELIMINADO );  " +
        ...
        public static bool Update(SqlConnection cnx, Piso em)
        {
            ...
            string SQL = string.Format("UPDATE {0} SET IDUSER= @IDUSER,PAIS=@PAIS, CIUDAD= @CIUDAD, POBLACION= @POBLACION, CALLE= @CALLE, CODPOS= @CODPOS," +
                " PRECIO= @PRECIO, OCUPANTES= @OCUPANTES, ALQUILADO= @ALQUILADO, DESRIPCION= @DESRIPCION, PUNTUACION= @PUNTUACION, ELIMINADO= @ELIMINADO  WHERE ID= @ID ; ",
                PisoCAD.TableName);
        ...
        public static Piso[] GetByCiudad(SqlConnection cnx, string ciudad)
        {
        ...
        public static Piso[] GetByPoblacion(SqlConnection cnx, string poblacion)
        {
            List<Piso> ls = new List<Piso>();
        ...
        public static Piso[] GetByCodpos(SqlConnection cnx, int codpos)
        {
            List<Piso> ls = new List<Piso>();
